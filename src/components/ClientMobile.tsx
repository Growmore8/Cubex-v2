  ...
  const Stepper = ({ small }: { small?: boolean }) => (
    <div className="flex flex-col items-center">
      <div className="flex items-center gap-2">
        <button onClick={() => setVol((v: number) => Math.max(0.01, +(v - 0.01).toFixed(2)))} className="ui-transition flex items-center justify-center rounded-full border border-[var(--border)] bg-[var(--soft)] active:scale-90" style={{ width: small ? 30 : 34, height: small ? 30 : 34 }}>−</button>
        <span className="font-semibold tabular-nums" style={{ minWidth: 44, textAlign: "center" }}>{Number(vol).toFixed(2)}</span>
        <button onClick={() => setVol((v: number) => +(v + 0.01).toFixed(2))} className="ui-transition flex items-center justify-center rounded-full border border-[var(--border)] bg-[var(--soft)] active:scale-90" style={{ width: small ? 30 : 34, height: small ? 30 : 34 }}>+</button>
      </div>
    ...
    <div style={{ ...(theme === "dark" ? DARK : LIGHT), fontFamily: "system-ui, -apple-system, 'Segoe UI', sans-serif", height: "100dvh", paddingTop: "env(safe-area-inset-top)" }} className="flex flex-col overflow-hidden bg-[var(--bg)] text-[var(--text)]">
      ...
      <div className="glass sticky top-0 z-20 flex items-center justify-between border-b px-3 py-2.5" style={{ borderColor: "var(--border)", background: theme === "dark" ? "rgba(20,24,34,0.6)" : "rgba(255,255,255,0.6)" }}>
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="ui-transition relative flex h-8 w-8 items-center justify-center rounded-full bg-[var(--soft)] active:scale-90">
      ...
      <div className="min-h-0 flex-1 overflow-auto">
        ...
        <KeepAlive active={tab === "dashboard"}>{(
          <div className="ui-fade-up space-y-4 p-3">
            ...
            {account?.type === "LIVE" ? (
              <div className="grid grid-cols-3 gap-2">
                <button onClick={() => setWalletTab("deposit")} className="hover-lift ui-transition flex flex-col items-center gap-1 rounded-xl py-3 text-white active:scale-[0.97]" style={{ background: BUY }}><i className="fa-solid fa-arrow-down" /><span className="text-[11px] font-semibold">Deposit</span></button>
                <button onClick={() => setWalletTab("withdraw")} className="hover-lift ui-transition flex flex-col items-center gap-1 rounded-xl py-3 text-white active:scale-[0.97]" style={{ background: SELL }}><i className="fa-solid fa-arrow-up" /><span className="text-[11px] font-semibold">Withdraw</span></button>
                <button onClick={() => { setXfer({ ...(xfer || {}), fromId: accId }); setXferModal(true); }} className="hover-lift ui-transition flex flex-col items-center gap-1 rounded-xl py-3 text-white active:scale-[0.97]" style={{ background: BLUE }}><i className="fa-solid fa-right-left" /><span className="text-[11px] font-semibold">Transfer</span></button>
              </div>
            ) : (
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[1000, 5000, 10000].map((amt) => (
                    <button key={amt} onClick={() => doTopUp(amt)} className="hover-lift ui-transition flex flex-col items-center gap-0.5 rounded-xl py-3 font-semibold active:scale-[0.97]" style={{ background: "rgba(240,180,41,0.14)", color: GOLD, border: "1px solid rgba(240,180,41,0.4)" }}>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    {(gainers.length ? gainers : priced.slice(0, 3)).map((s: any) => {
                      ...
                      return (
                        <button key={"g" + s.symbol} onClick={() => { setSelSym(s.symbol); setTab("chart"); }} className="ui-row -mx-1.5 flex w-full items-center gap-2 rounded-lg px-1.5 py-1.5">
                  ...
                  <div>
                    ...
                    {(losers.length ? losers : priced.slice(3, 6)).map((s: any) => {
                      const p = pctOf(s.symbol);
                      return (
                        <button key={"l" + s.symbol} onClick={() => { setSelSym(s.symbol); setTab("chart"); }} className="ui-row -mx-1.5 flex w-full items-center gap-2 rounded-lg px-1.5 py-1.5">
        ...
        <KeepAlive active={tab === "quotes"}>{(
          <div className="ui-fade-up p-3">
            <div className="relative mb-3">
              <i className="fa-solid fa-magnifying-glass absolute left-3 top-1/2 z-10 -translate-y-1/2 text-[var(--muted)]" />
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search symbols" className="ui-input w-full bg-[var(--card)] py-2.5 pl-9 pr-3 text-sm text-[var(--text)]" />
            </div>
            <div className="mb-3 flex gap-4 overflow-x-auto pb-1 text-[12px]" style={{ scrollbarWidth: "none" }}>
              <button onClick={() => setQcat("favs")} className="whitespace-nowrap pb-1 font-semibold transition-colors duration-200" style={{ color: qcat === "favs" ? BLUE : "var(--muted)", borderBottom: qcat === "favs" ? `2px solid ${BLUE}` : "2px solid transparent" }}>★ Favourites</button>
              {cats.map((c) => (
                <button key={c} onClick={() => setQcat(c)} className="whitespace-nowrap pb-1 font-semibold transition-colors duration-200" style={{ color: qcat === c ? BLUE : "var(--muted)", borderBottom: qcat === c ? `2px solid ${BLUE}` : "2px solid transparent" }}>{c}</button>
              ))}
            </div>
            <div className="ui-fade-up-stagger space-y-2.5">
              {quoteList.length === 0 ? <div className="py-6 text-center text-[12px] text-[var(--muted)]">No symbols.</div> : quoteList.map((s: any) => {
                ...
                return (
                  <div key={s.symbol} className="ui-card p-3" style={{ borderColor: dr > 0 ? BUY : dr < 0 ? SELL : undefined, transition: "border-color 0.4s ease" }}>
                    ...
                    <div className="grid grid-cols-3 items-center gap-2">
                      <button onClick={() => { setSelSym(s.symbol); quickTrade(s.symbol, "SELL"); }} className="ui-transition rounded-xl py-2 text-center text-white active:scale-[0.97]" style={{ background: SELL }}>
                        <div className="text-[10px] opacity-80">SELL</div><div className="text-sm font-bold tabular-nums">{sBid != null ? sBid.toFixed(dd) : "…"}</div>
                      </button>
                      <Stepper small />
                      <button onClick={() => { setSelSym(s.symbol); quickTrade(s.symbol, "BUY"); }} className="ui-transition rounded-xl py-2 text-center text-white active:scale-[0.97]" style={{ background: BUY }}>
        ...
        <KeepAlive active={tab === "chart"}>{(
          <div className="flex h-full flex-col">
            <div className="flex items-center gap-2 border-b border-[var(--border)] bg-[var(--panel)] px-3 py-2">
              ...
              <button onClick={() => { setSymSearch(""); setSymPickerOpen(true); }} className="ui-btn flex max-w-[180px] items-center gap-1.5 px-2.5 py-1 text-sm font-bold text-[var(--text)]">
              ...
              <div className="ml-auto flex items-center gap-1.5">
                <select value={tf} onChange={(e) => setTf(e.target.value)} className="ui-input px-1.5 py-1 text-[11px] text-[var(--text)]">
                  {(TFS || []).map((x: string) => <option key={x} value={x}>{x}</option>)}
                </select>
                <select value="" onChange={(e) => { if (e.target.value) setMInd((a) => a.includes(e.target.value) ? a : [...a, e.target.value]); }} className="ui-input px-1.5 py-1 text-[11px] text-[var(--text)]">
                  <option value="">+ Indicator</option>
                  {INDS.map(([label, id]) => <option key={id} value={id}>{label}</option>)}
                </select>
                <button onClick={() => setMInd([])} className="ui-btn ui-btn-ghost flex h-7 w-7 items-center justify-center text-[var(--muted)]"><i className="fa-solid fa-trash text-[11px]" /></button>
                <button onClick={() => setMInd([])} className="ui-btn ui-btn-ghost flex h-7 w-7 items-center justify-center text-[var(--muted)]"><i className="fa-solid fa-xmark" /></button>
              </div>
            </div>
            ...
            <div className="glass flex items-stretch gap-2 border-t border-[var(--border)] p-2.5" style={{ background: theme === "dark" ? "rgba(20,24,34,0.6)" : "rgba(255,255,255,0.6)" }}>
              <button onClick={() => quickTrade(selSym, "SELL", vol)} disabled={!account || account?.locked} className="ui-transition flex-1 rounded-xl py-2.5 text-center text-white active:scale-[0.98] disabled:opacity-50" style={{ background: SELL }}>
              ...
              <button onClick={() => quickTrade(selSym, "BUY", vol)} disabled={!account || account?.locked} className="ui-transition flex-1 rounded-xl py-2.5 text-center text-white active:scale-[0.98] disabled:opacity-50" style={{ background: BUY }}>
        ...
        {/* ───────── TRADES ───────── */}
        {tab === "trades" && (
          <div className="ui-fade-up space-y-3 p-3">
            <button onClick={() => { setNoForm({ idx: 0, lots: vol || 0.01, trigger: "", sl: "", tp: "" }); setNoOpen(true); }} className="ui-btn ui-btn-primary w-full py-3 text-sm font-semibold"><i className="fa-solid fa-plus mr-1.5" /> New Order / Pending</button>
            ...
            {(positions || []).length === 0 ? <div className="py-4 text-center text-[12px] text-[var(--muted)]">No open positions.</div> : (positions || []).map((p: any) => {
              ...
              return (
                <div key={p.id} className="ui-card overflow-hidden" style={{ borderLeft: `4px solid ${p.type === "BUY" ? BLUE : SELL}` }}>
                  {/* Tap the row to open/close trade details (no separate arrow) */}
                  <div onClick={() => setExpanded(open ? null : p.id)} className="ui-row flex cursor-pointer select-none items-center justify-between p-3">
                    <div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button onClick={(e) => { e.stopPropagation(); close(p.id); }} className="ui-transition flex h-7 w-7 items-center justify-center rounded-full border active:scale-90" style={{ borderColor: SELL, color: SELL }}><i className="fa-solid fa-xmark" /></button>
                    </div>
                  </div>
                  {open && (
                    <div className="border-t border-[var(--border)] p-3">
                      ...
                      <div className="mt-3 grid grid-cols-2 gap-2">
                        <button onClick={() => { setModifyId(p.id); setMSl(p.sl ? String(p.sl) : ""); setMTp(p.tp ? String(p.tp) : ""); }} className="ui-btn py-2 text-[12px] font-semibold">✎ Modify TP/SL</button>
                        <button onClick={() => close(p.id)} className="ui-transition rounded-xl py-2 text-[12px] font-semibold text-white active:scale-[0.98]" style={{ background: SELL }}>✕ Close</button>
                      </div>
                      {modifyId === p.id && (
                        <div className="ui-pop mt-3 rounded-xl border border-[var(--border)] bg-[var(--soft)] p-3">
                          <div className="mb-2 text-[11px] font-semibold">Modify SL / TP</div>
                          <div className="grid grid-cols-2 gap-2">
                            <input value={mSl} onChange={(e) => setMSl(e.target.value)} placeholder="Stop loss" className="ui-input bg-[var(--bg)] px-2 py-2 text-[12px] text-[var(--text)]" />
                            <input value={mTp} onChange={(e) => setMTp(e.target.value)} placeholder="Take profit" className="ui-input bg-[var(--bg)] px-2 py-2 text-[12px] text-[var(--text)]" />
                          </div>
                          <div className="mt-2 flex gap-2">
                            <button onClick={() => setModifyId(null)} className="ui-btn flex-1 py-2 text-[12px]">Cancel</button>
                            <button onClick={() => saveModify(p.id)} className="ui-btn ui-btn-primary flex-1 py-2 text-[12px] font-semibold">Save</button>
                          </div>
                        </div>
                      )}
                    </div>
            ...
            {(pending || []).map((o: any) => {
              ...
              return (
                <div key={o.id} className="ui-card p-3" style={{ borderStyle: "dashed", borderColor: c, borderLeftWidth: 4, borderLeftStyle: "solid" }}>
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => cancelPending(o.id)} className="ui-transition flex h-7 w-7 items-center justify-center rounded-full border active:scale-90" style={{ borderColor: SELL, color: SELL }}><i className="fa-solid fa-xmark" /></button>
                  </div>
        ...
        {tab === "history" && (
          <div className="ui-fade-up p-3">
            <div className="mb-3 flex gap-2">
              <button onClick={() => setHistTab("trades")} className="ui-transition flex-1 rounded-xl py-2 text-[12px] font-semibold" style={{ background: histTab === "trades" ? BLUE : "var(--soft)", color: histTab === "trades" ? "#fff" : "var(--muted)" }}>Trades</button>
              <button onClick={() => setHistTab("financial")} className="ui-transition flex-1 rounded-xl py-2 text-[12px] font-semibold" style={{ background: histTab === "financial" ? BLUE : "var(--soft)", color: histTab === "financial" ? "#fff" : "var(--muted)" }}>Financial</button>
            </div>
            {histTab === "trades" ? (
              <div className="ui-fade-up-stagger space-y-2.5">
                {(history || []).length === 0 ? <div className="py-6 text-center text-[12px] text-[var(--muted)]">No closed trades.</div> : (history || []).map((h: any) => {
                  ...
                  return (
                    <div key={h.id} className="ui-card p-3">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
              ...
              <div className="ui-fade-up-stagger space-y-2.5">
                {(financials || []).length === 0 ? <div className="py-6 text-center text-[12px] text-[var(--muted)]">No financial records.</div> : (financials || []).map((f: any) => {
                  ...
                  return (
                    <div key={f.id} className="ui-card flex items-center justify-between p-3">
                      <div>
          ...
          <div className="ui-fade-up space-y-4 p-3">
            {needKyc && (
              <div className="rounded-2xl border p-4" style={{ borderColor: "rgba(240,180,41,0.5)", background: "rgba(240,180,41,0.1)" }}>
                ...
                <button onClick={() => setWalletTab("kyc")} className="ui-transition mt-3 w-full rounded-xl py-2.5 text-sm font-semibold text-white active:scale-[0.98]" style={{ background: "#f0b829" }}>Upload KYC</button>
                {demoAccts.length > 0 && <button onClick={() => { const d = demoAccts.find((a: any) => a.id !== accId) || demoAccts[0]; if (d) switchAcc(d.id); }} className="ui-btn mt-2 w-full py-2 text-[12px] font-semibold">Switch to demo account</button>}
              </div>
            )}
            {/* header block */}
            ...
            <div className="ui-card p-4">
              <div className="mb-2 flex items-center justify-between">
                ...
                <select value={accId} onChange={(e) => switchAcc(e.target.value)} className="ui-input px-2 py-1 text-[11px] text-[var(--text)]">
            ...
            <div className="ui-card p-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="hover-lift rounded-xl p-3" style={{ background: "rgba(22,163,74,.1)" }}>
                ...
                <div className="hover-lift rounded-xl p-3" style={{ background: "rgba(220,38,38,.1)" }}>
            ...
            {Object.keys(bySym).length > 0 && (
              <div className="ui-card p-4">
                ...
                {Object.entries(bySym).map(([sym, arr]) => {
                  ...
                  const pl = sumPL(arr);
                  return (
                    <div key={sym} className="ui-row -mx-2 flex items-center justify-between rounded-lg px-2 py-1.5">
                      <div>
            ...
            <div className="ui-card p-4">
              ...
              {liveAccts.map((a: any) => (
                <button key={a.id} onClick={() => a.id !== accId && switchAcc(a.id)} className="ui-row -mx-2 flex w-full items-center gap-2 rounded-lg px-2 py-2 text-left">
              ...
              <button onClick={() => openAccount("LIVE")} className="ui-transition mt-2 w-full rounded-xl py-2 text-[12px] font-semibold text-white active:scale-[0.98]" style={{ background: BUY }}><i className="fa-solid fa-plus mr-1" /> Create New Live Account</button>
            </div>
            ...
            <div className="ui-card p-4">
              ...
              {demoAccts.length === 0 && (
                <button onClick={() => openAccount("DEMO")} className="ui-btn ui-btn-primary mt-2 w-full py-2 text-[12px] font-semibold"><i className="fa-solid fa-plus mr-1" /> Create Demo Account</button>
              )}
            </div>
            ...
            <div className="ui-card p-4">
              ...
              <button onClick={openPin} className="ui-row -mx-2 flex w-full items-center gap-3 rounded-lg px-2 py-2.5 text-left">
              ...
              <button onClick={addPasskey} className="ui-row -mx-2 flex w-full items-center gap-3 rounded-lg px-2 py-2.5 text-left">
              ...
              <button onClick={enablePush} className="ui-row -mx-2 flex w-full items-center gap-3 rounded-lg px-2 py-2.5 text-left">
                <i className="fa-solid fa-bell text-[var(--muted)]" />
              ...
              <button onClick={toggleTheme} className="ui-row -mx-2 flex w-full items-center gap-3 rounded-lg px-2 py-2.5 text-left">
            ...
            {/* export */}
            <button onClick={() => { try { window.open("/api/client/statement?accountId=" + encodeURIComponent(accId || ""), "_blank"); } catch { window.print(); } }} className="ui-card flex w-full items-center gap-3 p-4 text-left">
            ...
            {/* logout */}
            <button onClick={logout} className="ui-transition w-full rounded-xl py-3 text-sm font-semibold text-white active:scale-[0.98]" style={{ background: SELL }}><i className="fa-solid fa-right-from-bracket mr-1.5" /> Logout</button>
          </div>
        )}
      </div>
      ...
      {noOpen && (() => {
        ...
        const inp = "ui-input w-full px-3 py-2 text-sm tabular-nums";
        const ist = { background: "var(--bg)", color: "var(--text)" };
        return (
          <div className="ui-pop fixed inset-0 z-[95] flex items-end justify-center" style={{ background: "rgba(0,0,0,0.5)" }}>
            <div className="glass glass-edge w-full rounded-t-[26px] p-4" style={{ background: theme === "dark" ? "rgba(28,30,38,0.82)" : "rgba(255,255,255,0.82)", borderTop: "1px solid var(--border)" }} onClick={(e) => e.stopPropagation()}>
              <div className="mb-3 flex items-center justify-between">
                ...
                <button onClick={() => setNoOpen(false)} className="ui-btn ui-btn-ghost flex h-8 w-8 items-center justify-center text-[var(--muted)]"><i className="fa-solid fa-xmark" /></button>
              </div>
              <div className="mb-3 flex gap-1 rounded-xl border border-[var(--border)] p-1">
                {([["trade", "Trade"], ["pending", "Pending"]] as const).map(([k, lbl]) => (
                  <button key={k} onClick={() => setNoForm({ ...noForm, tab: k })} className="ui-transition flex-1 rounded-lg py-2 text-[12px] font-semibold" style={tab === k ? { background: BLUE, color: "#fff" } : { color: "var(--muted)" }}>{lbl}</button>
                ))}
              </div>
      ...
      {xferModal && (
        <div className="fixed inset-0 z-[90] flex items-center justify-center p-6" style={{ background: "rgba(0,0,0,0.6)" }}>
          <div className="ui-pop glass glass-edge w-full max-w-[340px] rounded-[22px] border p-4" style={{ background: theme === "dark" ? "rgba(28,30,38,0.85)" : "rgba(255,255,255,0.85)", borderColor: "var(--border)", color: "var(--text)" }} onClick={(e) => e.stopPropagation()}>
            <div className="mb-3 text-sm font-bold">Transfer Funds</div>
            <label className="text-[10px] text-[var(--muted)]">From</label>
            <select value={xfer?.fromId || ""} onChange={(e) => setXfer({ ...(xfer || {}), fromId: e.target.value })} className="ui-input mb-2 mt-1 w-full bg-[var(--bg)] px-2 py-2 text-[12px] text-[var(--text)]">
              {(accts || []).map((a: any) => <option key={a.id} value={a.id}>{a.login} ({a.type})</option>)}
            </select>
            <label className="text-[10px] text-[var(--muted)]">To</label>
            <select value={xfer?.toId || ""} onChange={(e) => setXfer({ ...(xfer || {}), toId: e.target.value })} className="ui-input mb-2 mt-1 w-full bg-[var(--bg)] px-2 py-2 text-[12px] text-[var(--text)]">
            ...
            <input type="number" value={xfer?.amount || ""} onChange={(e) => setXfer({ ...(xfer || {}), amount: e.target.value })} placeholder="0.00" className="ui-input mt-1 w-full bg-[var(--bg)] px-2 py-2 text-[12px] text-[var(--text)]" />
            {xferErr && <div className="mt-2 text-[11px]" style={{ color: SELL }}>{xferErr}</div>}
            <div className="mt-3 flex gap-2">
              <button onClick={() => setXferModal(false)} className="ui-btn flex-1 py-2 text-[12px]">Cancel</button>
              <button onClick={doTransfer} className="ui-btn ui-btn-primary flex-1 py-2 text-[12px] font-semibold">Transfer</button>
            </div>
          </div>
        </div>
      )}
      ...
      {walletTab && (
        <div className="fixed inset-0 z-[125] flex items-start justify-center overflow-auto p-3" style={{ background: "rgba(0,0,0,0.55)", paddingTop: "max(12px, env(safe-area-inset-top))", paddingBottom: "max(12px, env(safe-area-inset-bottom))" }}>
          <div className="ui-pop w-full max-w-md rounded-2xl bg-white p-4 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="fixed inset-0 z-[115] flex flex-col" style={{ background: "var(--bg)", paddingTop: "env(safe-area-inset-top)" }}>
          <div className="flex items-center gap-2 border-b border-[var(--border)] bg-[var(--panel)] px-3 py-2.5">
            <div className="relative flex-1">
              <i className="fa-solid fa-magnifying-glass absolute left-3 top-1/2 z-10 -translate-y-1/2 text-[var(--muted)]" />
              <input autoFocus value={symSearch} onChange={(e) => setSymSearch(e.target.value)} placeholder="Search symbols" className="ui-input w-full bg-[var(--card)] py-2.5 pl-9 pr-3 text-sm text-[var(--text)]" />
            </div>
            <button onClick={() => setSymPickerOpen(false)} aria-label="Close" className="ui-btn ui-btn-ghost flex h-9 w-9 items-center justify-center rounded-full text-[var(--muted)]"><i className="fa-solid fa-xmark text-lg" /></button>
          </div>
          <div className="flex-1 overflow-auto">
            {(symbols || []).filter((x: any) => { const q = symSearch.trim().toLowerCase(); if (!q) return true; return `${x.display || x.symbol}`.toLowerCase().includes(q) || `${x.symbol}`.toLowerCase().includes(q); }).map((x: any) => {
              ...
              return (
                <button key={x.symbol} onClick={() => { setSelSym(x.symbol); setSymPickerOpen(false); }} className="ui-row flex w-full items-center justify-between border-b border-[var(--border)] px-4 py-3 text-left" style={x.symbol === selSym ? { background: "var(--soft)" } : undefined}>
      ...
      {pin?.pinModal && (
        <div className="fixed inset-0 z-[95] flex items-center justify-center p-6" style={{ background: "rgba(0,0,0,0.5)" }}>
          <div className="ui-pop glass glass-edge w-full max-w-[320px] rounded-[22px] border p-4" style={{ background: theme === "dark" ? "rgba(28,30,38,0.85)" : "rgba(255,255,255,0.85)", borderColor: "var(--border)", color: "var(--text)" }} onClick={(e) => e.stopPropagation()}>
            <div className="mb-2 flex items-center justify-between">
              <div className="text-sm font-semibold">{pin.pinHasPin ? "Change PIN" : "Set PIN"}</div>
              <button onClick={() => pin.setPinModal(false)} aria-label="Close" className="ui-btn ui-btn-ghost -mr-1 flex h-7 w-7 items-center justify-center rounded-full text-[var(--muted)]"><i className="fa-solid fa-xmark" /></button>
            </div>
            {pin.pinHasPin && (<><div className="text-[10px] text-[var(--muted)]">Current PIN</div><input type="password" inputMode="numeric" value={pin.pinForm.current || ""} onChange={(e) => pin.setPinForm({ ...pin.pinForm, current: e.target.value })} className="ui-input mb-2 mt-1 w-full bg-[var(--bg)] px-2 py-2 text-center text-[var(--text)]" /></>)}
            <div className="text-[10px] text-[var(--muted)]">New PIN (4-6 digits)</div>
            <input type="password" inputMode="numeric" value={pin.pinForm.pin || ""} onChange={(e) => pin.setPinForm({ ...pin.pinForm, pin: e.target.value })} className="ui-input mt-1 w-full bg-[var(--bg)] px-2 py-2 text-center text-[var(--text)]" />
            {pin.pinErr && <div className="mt-2 text-[10px]" style={{ color: SELL }}>{pin.pinErr}</div>}
            <button onClick={pin.savePin} className="ui-transition mt-3 w-full rounded-xl py-2 text-xs font-semibold text-white active:scale-[0.98]" style={{ background: BUY }}>Save PIN</button>
          </div>
        </div>
      )}
      ...
      {pin?.pinLock && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-6" style={{ background: "rgba(5,9,16,0.96)" }}>
          <div className="ui-pop ui-card w-full max-w-[320px] p-5 text-center" style={{ background: "var(--panel)", color: "var(--text)" }}>
            ...
            <input type="password" inputMode="numeric" autoFocus value={pin.pinInput} onChange={(e) => pin.setPinInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") pin.unlock(); }} className="ui-input w-full bg-[var(--bg)] px-2 py-2 text-center text-lg tracking-widest text-[var(--text)]" />
            {pin.pinErr && <div className="mt-2 text-[10px]" style={{ color: SELL }}>{pin.pinErr}</div>}
            <button onClick={pin.unlock} className="ui-transition mt-3 w-full rounded-xl py-2 text-xs font-semibold text-white active:scale-[0.98]" style={{ background: BUY }}>Unlock</button>
            <button onClick={pin.unlockPasskey} className="ui-btn mt-2 w-full py-2 text-xs">Unlock with passkey</button>
          </div>
        </div>
      )}
    </div>
