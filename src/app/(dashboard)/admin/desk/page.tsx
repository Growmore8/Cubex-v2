...
export default function AdminDeskPage() {
  ...
  useEffect(() => { if (selAcc && selAcc.id) { fetch("/api/desk/history?accountId=" + selAcc.id).then((r) => r.json()).then((h) => { if (h.ok) setHistory(h.history); }).catch(() => {}); } }, [selAcc]);
  ...
  async function submitAct() {
    ...
    else if (act.kind === "rename") { url = "/api/admin/clients/" + id + "/manage"; body = { action: "rename", name: aform.name ?? act.acc.name, email: aform.email ?? act.acc.email, country: aform.country ?? act.acc.country }; }
  ...
  const acctRow = (c: any) => (
    <button key={c.id} onClick={() => setSelAcc(c)} onContextMenu={(e) => { e.preventDefault(); setSelAcc(c); setMenu({ x: e.clientX, y: e.clientY, acc: c }); }}
      ...
      {c.doNotLiquidate ? stTag("DNL", GOLD) : null}
      {c.locked ? stTag("LCK", SELL) : null}
      {c.deactivated ? stTag("OFF", "#8b97a8") : null}
      {c.kycStatus === "APPROVED" ? stTag("KYC", BUY) : c.kycStatus === "PENDING" ? stTag("KYC", GOLD) : c.kycStatus === "REJECTED" ? stTag("KYC", SELL) : null}
    </button>
  );
      ...
      {menu && (<>
        ...
        <div className="fixed z-50 w-56 overflow-visible rounded-md border text-[11px]" style={{ left: menu.x, top: menu.y, background: "var(--panel)", borderColor: "var(--border)", color: "var(--text)" }}>
          ...
          {menuSub === "edit" && (<div className="absolute left-full top-0 z-50 w-52 overflow-hidden rounded-md border" style={{ background: "var(--panel)", borderColor: "var(--border)" }}>
            ...
            <button onClick={() => doClearPin(menu.acc)} className={subi}>Reset PIN</button>
          </div>)}
