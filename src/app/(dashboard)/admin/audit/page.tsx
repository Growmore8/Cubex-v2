...
export default function AdminAuditPage() {
  ...
  return (
    <div className="space-y-6 ui-fade-up">
      ...
      <form onSubmit={broadcast} className="ui-card flex flex-wrap items-center gap-3 p-4">
        ...
        <button className="ui-btn ui-btn-primary px-4 py-2 text-sm">Send</button>
      ...
      <div className="ui-card overflow-x-auto p-0">
        <table className="w-full text-sm"><thead className="border-b bg-gray-50 text-left text-gray-600"><tr><th className="px-3 py-2">When</th><th className="px-3 py-2">By</th><th className="px-3 py-2">Action</th><th className="px-3 py-2">Detail</th></tr></thead>
          <tbody>{logs.length === 0 ? <tr><td className="px-3 py-4" colSpan={4}>No activity.</td></tr> : logs.map((l) => (
            <tr key={l.id} className="ui-row border-b last:border-0"><td className="px-3 py-2 text-xs text-gray-500">{new Date(l.createdAt).toLocaleString()}</td><td className="px-3 py-2">{l.performedBy}</td><td className="px-3 py-2 font-mono text-xs">{l.action}</td><td className="px-3 py-2 text-gray-600">{l.detail}</td></tr>
          ))}</tbody></table>
      </div>
    </div>
  );
