          ...
          <tbody>
            {docs.length === 0 ? <tr><td className="px-3 py-4" colSpan={6}>No documents.</td></tr> : docs.map((d) => (
              <tr key={d.id} className="border-b last:border-0">
                ...
                <td className="px-3 py-2"><a className="text-blue-600 underline" href={"/api/files/kyc/" + d.id} target="_blank" rel="noreferrer">View</a></td>
                <td className="px-3 py-2"><span className={badge(d.status)}>{d.status}</span></td>
                <td className="px-3 py-2"><input className="rounded border px-2 py-1 text-xs" placeholder="reason" value={note[d.id] || ""} onChange={(e) => setNote({ ...note, [d.id]: e.target.value })} /></td>
                <td className="px-3 py-2 text-right space-x-2">
                  <button className="text-green-600" onClick={() => review(d.id, "APPROVED")}>Approve</button>
                  <button className="text-red-600" onClick={() => review(d.id, "REJECTED")}>Reject</button>
                </td>
              </tr>
            ))}
          </tbody>
