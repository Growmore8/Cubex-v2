                      ...
                      <tbody>
                        {rows.length === 0 ? <tr><td className="px-2 py-3 text-[var(--muted)]" colSpan={16}>No history.</td></tr> : sortRows("hist", rows, {
                          ...
                          <tr key={h.id} className="border-b border-[var(--border)] hover:bg-[var(--soft)]">
                            ...
                            {(() => { const net = Number(h.pnl ?? 0) + (swapEnabled ? Number(h.swap ?? 0) : 0) - (swapEnabled ? Number(h.commission ?? 0) : 0); return <td className="px-1 py-0.5 text-right">{h.kind === "TRADE" ? <span className="inline-block rounded px-1.5 py-0.5 text-[10px] font-bold tabular-nums" style={{ background: net >= 0 ? "rgba(38,166,154,0.13)" : "rgba(239,83,80,0.13)", color: net >= 0 ? BUY : SELL }}>{(net >= 0 ? "+" : "") + gnum(net, 2)}</span> : <span style={{ color: "var(--muted)" }}>—</span>}</td>; })()}
