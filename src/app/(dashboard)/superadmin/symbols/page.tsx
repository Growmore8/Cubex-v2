...
export default function SuperadminSymbolsPage() {
  ...
  const Group = ({ title, cat }: any) => (
    <div className="ui-card bg-white p-3">
      <div className="mb-2 text-sm font-semibold capitalize">{title} ({byCat(cat).length})</div>
      <div className="flex flex-wrap gap-1.5">{byCat(cat).map((s) => <span key={s.symbol} className="rounded-lg bg-gray-100 px-2 py-1 font-mono text-xs transition-colors duration-200">{s.symbol}</span>)}</div>
    </div>
  );
  return (
