...
export default function SAPlatform() {
  ...
  return (<div className="max-w-4xl space-y-4 ui-fade-up">
    ...
    <div className="space-y-2 ui-fade-up-stagger">
      {MODES.map(([k, t, d, c]) => (<button key={k} onClick={() => save(k)} className="ui-transition hover-lift block w-full rounded-xl border p-4 text-left" style={{ borderColor: mode === k ? c : "#e2e8f0", background: mode === k ? c + "18" : "#fff" }}>
