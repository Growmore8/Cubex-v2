...
export default function SAClients() {
  const [deleteTarget, setDeleteTarget] = useState<null | { id: string; login: string }>(null);
  const [resetUser, setResetUser] = useState<null | { id: string; login: string }>(null);
  ...
  async function act(id: string, action: string, extra: any) {
    ...
    if (!d.ok) {
      ...
      if (action === "delete") toast.error(d.error || "Failed to delete client");
      if (action === "resetPassword") toast.error(d.error || "Failed to reset password");
      return;
    }
    if (action === "delete") toast.success("Client deleted successfully");
    if (action === "resetPassword") toast.success("Password reset successfully");
    load();
  }
  async function applyMoney() {
    ...
    if (!d.ok) {
      setErr(d.error || "Failed");
      toast.error(d.error || "Failed to update balance");
      return;
    }
    ...
    <>
                ...
                {filtered.map((r: any) => (<tr key={r.id} className="border-t" style={{ borderColor: "#eef2f7" }}>
                  ...
                  <td className="px-2 py-2 space-x-1 text-right">
                    ...
                    <button
                      ...
                      onClick={() => {
                        setResetUser({ id: r.id, login: r.login });
                        setNewPassword("");
                      }}
                    >
                    ...
                    <button
                      ...
                      onClick={() => setDeleteTarget({ id: r.id, login: r.login })}
                    >
      ...
      <Dialog open={!!resetUser} onOpenChange={() => setResetUser(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Reset Password for {resetUser?.login}
            </DialogTitle>
          </DialogHeader>
            ...
            <Button
              ...
              onClick={() => {
                if (resetUser && newPassword.length >= 6) {
                  act(resetUser.id, "resetPassword", {
