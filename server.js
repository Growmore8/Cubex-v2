...
async function pollChunk(chunk) {
  // chunk: array of internal symbols. Map to TD symbols (dedupe), fetch bid/ask + price.
  const uniqTd = Array.from(new Set(chunk.map((s) => meta[s].td)));
  try {
    // Use /quote instead of /price to get real bid + ask for forex symbols
    const url = "https://api.twelvedata.com/quote?symbol=" + encodeURIComponent(uniqTd.join(",")) + "&apikey=" + TD_KEY;
    const res = await fetch(url);
    const data = await res.json();
    if (!data) return;
    // Single-symbol response: { close/price, bid?, ask? }
    const applyEntry = (s, entry) => {
      ...
      if (ask > 0) {
        if (bid > 0 && bid < ask) {
          // Real bid/ask from TD (forex): store bid in state, use ask as display price
          state[s].bid = r(bid, meta[s].digits);
          applyPrice(s, ask, "TD");
        } else {
          state[s].bid = null; // no real bid — will be calculated from spread
          applyPrice(s, ask, "TD");
        }
      }
    };
    if (uniqTd.length === 1) {
      for (const s of chunk) if (meta[s].td === uniqTd[0]) applyEntry(s, data);
      return;
    }
